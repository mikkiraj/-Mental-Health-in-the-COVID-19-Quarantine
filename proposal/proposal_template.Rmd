...
cleancovid %>%
  filter(Dem_gender %in% c("Male", 
                           "Female")) %>%
  ggplot(aes(x = Dem_gender, 
             fill = Dem_gender ))+ 
  geom_bar() +
...
cleancovid %>%
  filter(Dem_isolation %in% c("Isolated", 
                              "Life carries on with minor changes", 
                              "Life carries on as usual")) %>%
  ggplot(aes(x = Dem_isolation, 
             fill = Dem_isolation)) + 
  geom_bar() + 
...
```{r explanatoryvariables-PSSmeans1}
...
```{r explanatoryvariables-PSSmeanscalculation}
...
```{r explanatoryvariables-SLONmeans1}
...
```{r explanatoryvariables-SLONmeanscalculation}
...
```{r explanatoryvariables-SPSmeans1}
...
```{r explanatoryvariables-SPSmeanscalculation}
...
```{r ttest-gender}
genderPSSttest <- t.test(covidforanalysis$PSSmeans~cleancovid$Dem_gender, var.equal=FALSE)
genderSLONttest <- t.test(covidforanalysis2$SLONmeans~cleancovid$Dem_gender, var.equal=FALSE)
genderSPSttest <- t.test(covidforanalysis3$SPSmeans~cleancovid$Dem_gender, var.equal=FALSE)
```

```{r ttest-table}
ttesttab <- matrix(c(1.75*10^-309,1.16*10^-135, 1.98*10^-278, 0.114, 0.221, 0.3, 0.16, 0.259, 0.335), ncol=3, byrow=TRUE)
colnames(ttesttab) <- c('PSS','SLON','SPS')
rownames(ttesttab) <- c('P values', 'Confidence Intervals Low', 'Confidence Intervals High')
tttesttab <- as.table(ttesttab)
knitr::kable(ttesttab,digits=10,caption="T-Test Results for Gender and PSS, SLON, and SPS Scales") #when we knit, see if the title shows up 
print(ttesttab)
```


```{r anova-isolation}
anovaPSSisolation <- summary(aov(covidforanalysis$PSSmeans~cleancovid$Dem_isolation))
anovaSLONisolation <-summary(aov(covidforanalysis2$SLONmeans~cleancovid$Dem_isolation))
anovaSPSisolation <- summary(aov(covidforanalysis3$SPSmeans~cleancovid$Dem_isolation))
```

```{r anova-marital}
anovaPSSmarital <- summary(aov(covidforanalysis$PSSmeans~cleancovid$Dem_maritalstatus))
anovaSLONmarital <-summary(aov(covidforanalysis2$SLONmeans~cleancovid$Dem_maritalstatus))
anovaSPSmarital <- summary(aov(covidforanalysis3$SPSmeans~cleancovid$Dem_maritalstatus))
```

```{r anova-employment}
anovaPSSemploy <- summary(aov(covidforanalysis$PSSmeans~cleancovid$Dem_employment))
anovaSLONemploy <-summary(aov(covidforanalysis2$SLONmeans~cleancovid$Dem_employment))
anovaSPSemploy <- summary(aov(covidforanalysis3$SPSmeans~cleancovid$Dem_employment))
```

```{r anova-dependents}
anovaPSSdepend <- summary(aov(covidforanalysis$PSSmeans~cleancovid$Dem_dependentsgrouped))
anovaSLONdepend <-summary(aov(covidforanalysis2$SLONmeans~cleancovid$Dem_dependentsgrouped))
anovaSPSdepend <- summary(aov(covidforanalysis3$SPSmeans~cleancovid$Dem_dependentsgrouped))
```

```{r anova-table}
anovatab <- matrix(c(0, 0, 0, 
                     1.22*10^-10, 1.48*10^-298, 0, 
                     3.52*10^-135, 6.02*10^-160, 2.14*10^-123, 
                     8.79*10^-21, 4.12*10^-63, 6.11*10^-27), ncol=3, byrow=TRUE) 
colnames(anovatab) <- c('PSS','SLON','SPS')
rownames(anovatab) <- c('Isolation Status', 'Marital Status', 'Employment', 'Dependents')
anovatab <- as.table(anovatab)
knitr::kable(anovatab,digits=10,caption="P Values for ANOVA Results") #when we knit, see if the title shows up 
print(anovatab)
```
...
covid_PSSage <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~cleancovid$Dem_age, data = cleancovid)
  covid_PSSage_tidy <- tidy(covid_PSSage, conf.int=TRUE)
  print(covid_PSSage_tidy)
  
covid_SLONage <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis2$SLONmeans~cleancovid$Dem_age, data = cleancovid)
  covid_SLONage_tidy <- tidy(covid_SLONage, conf.int = TRUE)
  print(covid_SLONage_tidy)
  
covid_SPSage <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis3$SPSmeans~cleancovid$Dem_age, data = cleancovid)
  covid_SPSage_tidy <- tidy(covid_SPSage, conf.int = TRUE)
  print(covid_SPSage_tidy)
```

```{r lin-reg-table}
lin_reg_table <- matrix(c(-0.0047, -0.0128, -0.0008,
                          7.41*10^-267, 0, 0.0045, 
                          -0.0049,-0.0135, -0.0014
                          -0.0044, -0.0122, -0.0003), 
                          ncol=3, byrow=TRUE)
colnames(lin_reg_table) <- c('PSS','SLON','SPS')
...
knitr::kable(lin_reg_table,digits=10,caption="Linear Regression Results for Age and PSS, SLON, and SPS Scales") #when we knit, see if the title shows up 
print(lin_reg_table)
```

```{r lin-reg-equations-table}
lin_reg_equations_table <- matrix(c('y_expected = 3.1628 - 0.00465(age)', 
                                    'y_expected = 3.0734 - 0.01281(age)', 
                                    'y_expected = 4.9504 - 0.000805(age)'),  
                          ncol=3, byrow=TRUE)
colnames(lin_reg_equations_table) <- c('PSS','SLON','SPS')
rownames(lin_reg_equations_table) <- c('Equation')
lin_reg_equations_table <- as.table(lin_reg_equations_table)
knitr::kable(lin_reg_equations_table,digits=10,caption="Linear Regression Results for Age and PSS, SLON, and SPS Scales") #when we knit, see if the title shows up 
...
ggplot(data = cleancovid, 
       aes(x = Dem_age, 
           y = covidforanalysis3$SPSmeans)) +
  geom_point(alpha=.5) +
  geom_smooth(method = "lm",
              se = FALSE, 
              color = "blue") +
  labs(
    title = "SPS means and Age",
    x = "Age",
    y = "SPS means"
      )
...
covid_PSS <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped, data = cleancovid)
  covid_PSS_tidy <- tidy(covid_PSS, conf.int=TRUE)
  print(covid_PSS_tidy)
  
covid_SLON <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis2$SLONmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped, data = cleancovid)
  covid_SLON_tidy <- tidy(covid_SLON, conf.int=TRUE)
  print(covid_SLON_tidy)
  
covid_SPS <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis3$SPSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped, data = cleancovid)
  covid_SPS_tidy <- tidy(covid_SPS, conf.int=TRUE)
  print(covid_SPS_tidy)  
```
...
SPS_R2 <- glance(covid_SPS)$adj.r.squared
```

```{r interaction-models1}
#PSS: marital status and isolation
maritalstatus_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_maritalstatus*Dem_isolation, data = cleancovid)
  maritalstatus_isolation_interaction_tidy <- tidy(maritalstatus_isolation_interaction, conf.int=TRUE)
  print(maritalstatus_isolation_interaction_tidy)
  maritalstatus_isolation_R2 <- glance(maritalstatus_isolation_interaction)$adj.r.squared
  print(maritalstatus_isolation_R2)
```

```{r interaction-models2}
#PSS: age and isolation 
 age_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_age*Dem_isolation, data = cleancovid)
  age_isolation_interaction_tidy <- tidy(age_isolation_interaction, conf.int=TRUE)
  print(age_isolation_interaction_tidy)
  age_isolation_R2 <- glance(age_isolation_interaction)$adj.r.squared 
  print(age_isolation_R2)
```

```{r interaction-models3}
#PSS: gender and isolation 
 gender_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_gender*Dem_isolation, data = cleancovid)
  gender_isolation_interaction_tidy <- tidy(gender_isolation_interaction, conf.int=TRUE)
  print(gender_isolation_interaction_tidy)
  gender_isolation_R2 <- glance(gender_isolation_interaction)$adj.r.squared 
  print(gender_isolation_R2)
```

```{r interaction-models4}
#PSS: dependents and isolation 
 dependents_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_dependentsgrouped*Dem_isolation, data = cleancovid)
  dependents_isolation_interaction_tidy <- tidy(dependents_isolation_interaction, conf.int=TRUE)
  print(dependents_isolation_interaction_tidy)
  dependents_isolation_R2 <- glance(dependents_isolation_interaction)$adj.r.squared 
  print(dependents_isolation_R2)
```


```{r interaction-models5}
#PSS: employment and isolation 
 employment_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_employment*Dem_isolation, data = cleancovid)
  employment_isolation_interaction_tidy <- tidy(employment_isolation_interaction, conf.int=TRUE)
  print(employment_isolation_interaction_tidy)
  employment_isolation_R2 <- glance(employment_isolation_interaction)$adj.r.squared 
  print(employment_isolation_R2)
```

```{r interaction-models6}
#PSS: age and gender
  age_gender_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis$PSSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_age*Dem_gender, data = cleancovid)
  age_gender_interaction_tidy <- tidy(age_gender_interaction, conf.int=TRUE)
  print(age_gender_interaction_tidy)
  age_gender_R2 <- glance(age_gender_interaction)$adj.r.squared 
  print(age_gender_R2)
```
...
```{r interaction-models7}
#SLON: age and gender
  SLON_age_gender_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis2$SLONmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_age*Dem_gender, data = cleancovid)
  SLON_age_gender_interaction_tidy <- tidy(SLON_age_gender_interaction, conf.int=TRUE)
  print(SLON_age_gender_interaction_tidy)
  SLON_age_gender_R2 <- glance(SLON_age_gender_interaction)$adj.r.squared 
  print(SLON_age_gender_R2)
```


```{r interaction-models8}
#SLON: employment and isolation
  SLON_employment_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis2$SLONmeans ~ Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_employment*Dem_isolation, data = cleancovid)
  SLON_employment_isolation_interaction_tidy <- tidy(SLON_employment_isolation_interaction, conf.int=TRUE)
  print(SLON_employment_isolation_interaction_tidy)
  SLON_employment_isolation_R2 <- glance(SLON_employment_isolation_interaction)$adj.r.squared 
  print(SLON_employment_isolation_R2)
```

```{r interaction-models9}
#SLON: marital and isolation
  SLON_marital_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis2$SLONmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_maritalstatus*Dem_isolation, data = cleancovid)
  SLON_marital_isolation_interaction_tidy <- tidy(SLON_marital_isolation_interaction, conf.int=TRUE)
  print(SLON_marital_isolation_interaction_tidy)
  SLON_marital_isolation_R2 <- glance(SLON_marital_isolation_interaction)$adj.r.squared 
  print(SLON_marital_isolation_R2)
```



```{r interaction-models10}
#SPS: age and gender 
  SPS_age_gender_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis3$SPSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_age*Dem_gender, data = cleancovid)
 SPS_age_gender_interaction_tidy <- tidy(SPS_age_gender_interaction, conf.int=TRUE)
  print(SPS_age_gender_interaction_tidy)
  SPS_age_gender_R2 <- glance(SPS_age_gender_interaction)$adj.r.squared 
  print(SPS_age_gender_R2)
```

```{r interaction-models11}
#SPS: employment and isolation 
  SPS_employment_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(covidforanalysis3$SPSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_employment*Dem_isolation, data = cleancovid)
  SPS_employment_isolation_interaction_tidy <- tidy(SPS_employment_isolation_interaction, conf.int=TRUE)
  print(SPS_employment_isolation_interaction_tidy)
  SPS_employment_isolation_R2 <- glance(SPS_employment_isolation_interaction)$adj.r.squared
  print(SPS_employment_isolation_R2)
```

```{r interaction-models12}
#SPS: marital and isolation 
  SPS_marital_isolation_interaction <- linear_reg() %>%
  set_engine("lm") %>%
  fit(log(covidforanalysis3)$SPSmeans~Dem_gender + Dem_age + Dem_maritalstatus + Dem_isolation + Dem_employment + Dem_dependentsgrouped + Dem_maritalstatus*Dem_isolation, data = cleancovid)
  SPS_employment_isolation_interaction_tidy <- tidy(SPS_employment_isolation_interaction, conf.int=TRUE)
  print(SPS_employment_isolation_interaction_tidy)
  SPS_employment_isolation_R2 <- glance(SPS_employment_isolation_interaction)$adj.r.squared
  print(SPS_employment_isolation_R2)
...
```{r}
covidforanalysis3%>%
  ggplot(aes(x = scale(SPSmeans))) + 
  geom_histogram()
```

```{r augmented}
augmented_model <- augment(SPS_marital_isolation_interaction$fit)

ggplot(augmented_model, mapping = aes(x = .fitted, y = .resid)) + 
geom_point(alpha = 0.5)+
geom_hline(yintercept=0, color = "blue")+
labs(title = "Residuals vs Predicted Log Hair Mercury", 
x = "Square Root of Hair Mercury", y = "Residuals")  
```

```{r R^2-table}
# <- matrix(c(), 
                          #ncol=3, byrow=TRUE)
#colnames(R2_table) <- c('Adjusted R^2 Value')
#rownames(R2_table) <- c('PSS:Age+Gender', 'PSS:Employment+Isolation', 'PSS:Marital+Isolation', 'PSS:Age+Isolation', 'PSS:Gender+Isolation', 'PSS:Dependents+Isolation', 'SLON:Age+Gender', 'SLON:Employment+Isolation', 'SLON:Marital+Isolation', 'SPS:Age+Gender', 'SPS:Employment+Isolation', 'SPS:Marital+Isolation')
#R2_table <- as.table(R2_table)
#knitr::kable(R2_table,digits=10,caption="Adjusted R^2 Values for PSS, SLON, and SPS Scales") #when we knit, see if the title shows up 
#print(R2_table)
```
