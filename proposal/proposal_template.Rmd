...
```{r filter-NA}
covidforanalysis <- subset (cleancovid, select = c(1:40)) %>%
...
```{r explanatoryvariables}
#covidforanalysistest <- subset (covidforanalysis, select = c(21:23)) %>%
 # Scale_PSS10_UCLA_1 <-as.numeric(as.character(covidforanalysistest$Scale_PSS10_UCLA_1))%>%
# Scale_PSS10_UCLA_2 <-as.numeric(as.character(covidforanalysistest$Scale_PSS10_UCLA_2))%>%
#covidforanalysistest$pssmean = rowSums(covidforanalysistest)
